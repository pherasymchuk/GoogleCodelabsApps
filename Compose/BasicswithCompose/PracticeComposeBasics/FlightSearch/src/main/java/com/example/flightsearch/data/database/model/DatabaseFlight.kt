...
@Entity(
    ...
    foreignKeys = [
        ForeignKey(entity = DatabaseAirport::class, parentColumns = ["id"], childColumns = ["departure_airport_id"]),
        ForeignKey(entity = DatabaseAirport::class, parentColumns = ["id"], childColumns = ["arrival_airport_id"])
    ],
    indices = [Index(value = ["departure_airport_id"]), Index(value = ["arrival_airport_id"])]
)
data class DatabaseFlight(
